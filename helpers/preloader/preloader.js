...
const Preloader = (_ => {
    let observer = null, timer;

    const throttleTime = 200, store = [], isIntersection = 'IntersectionObserver' in window, y = window.scrollY;

    return class {
        constructor(config = {}, images){
            if(!images.length) Helper.err('There are no images!');
        ...
        preload(target, index){
            ...
            if(!isIntersection) this.destroyStore(index);
        }
        ...
        repainting(target){
            ...
            target.tagName.toLowerCase() === 'img' ? target.src = preload : target.style.background.replace(/url\([?.*]\)/i, preload);
        }
